<!--
@author: wuyuxin
@description: 缓冲区查询，弹窗组件
@time: 2020/12/02
参数说明：
/**具体内容请想看props内部传递数据说明*/
-->
<template>
    <div class="emergency_disposal full_whh">
        <div id="map"></div>

        <dialog-zone

                :sliderShow="true"
                :imgUrl="imgUrl"
                :radius="'50'"
                @submitSearchAroundDialog="submitSearchAroundDialogS"
                @cancelSearchAroundDialog="cancelSearchAroundDialog"
                @chengeAroundChecked="chengeAroundChecked"
                :coverShow="coverShow"
                max-number="200"
                min-number="0"
                length-number="7"
                default-number="50"
                :currentPoint="currentPoint"
                :searchAroundDialogVisible="searchAroundDialogVisible">

        </dialog-zone>
        <!--        &lt;!&ndash;周边搜索弹窗start&ndash;&gt;-->
        <!--        <div class="map_dialog zbss_dialog zkr-dialog"-->
        <!--             v-show="searchAroundDialogVisible"-->
        <!--             :footerAlign="'center'"-->
        <!--             submitBtnText="查询">-->
        <!--            &lt;!&ndash;            //需要把elementui的内容替换掉&ndash;&gt;-->
        <!--            <div class="search_around_box">-->
        <!--                <div class="header_box">-->
        <!--                    <span class="spantitle">周边搜索</span>-->
        <!--                    <i @click="cancelSearchAroundDialog()" class="layui-icon layui-icon-close header_close"-->
        <!--                       style="font-size: 20px;"></i>-->
        <!--                </div>-->
        <!--                <div class="content">-->
        <!--                    <div class="around_item_box" v-for="(item, index) in searchAroundTypeArr" :key="'around_'+index">-->
        <!--                        <div class="around_count_box" :class="item.active?'active':''"-->
        <!--                             @click="chengeAroundChecked(index)">-->
        <!--                            <i class="layui-icon layui-icon-ok-circle" v-show="item.active"-->
        <!--                               style="font-size: 17px;position: absolute;left: 5px;top: 50%;transform: translateY(-50%); color: #1E9FFF;"></i>-->
        <!--                            <div class="around_img"><img :src="imgUrl+item.src"></div>-->
        <!--                            <div class="around_name">{{item.name}}</div>-->
        <!--                        </div>-->
        <!--                    </div>-->

        <!--                </div>-->
        <!--                &lt;!&ndash;                      oninput="value=value.replace(/^(([1-9][0-9]*)|(([0]\.\d{1,2}|[1-9][0-9]*\.\d{1,2})))/g,'')"&ndash;&gt;-->
        <!--                &lt;!&ndash;@input="inputNum"&ndash;&gt;-->
        <!--                <form class="layui-form layui-form-pane login_form" lay-filter="example">-->
        <!--                    <div class="around_input_box input_flex">-->
        <!--                        <input type="text" name="number" lay-verify="required|number|numbers" v-model="radius"-->
        <!--                               placeholder="请输入半径" autocomplete="off" class="layui-input">-->
        <!--                        <span class="show_title">千米</span>-->
        <!--                    </div>-->
        <!--                    <div class="slider">-->
        <!--                        <div id="slideTest3" class="demo-slider"></div>-->
        <!--                    </div>-->
        <!--                    <div class="layui-form-item">-->
        <!--                        <div class="layui-input-block">-->
        <!--                            <button type="button" class="layui-btn diabtn" lay-submit=""-->
        <!--                                    @click="cancelSearchAroundDialog()">取消-->
        <!--                            </button>-->
        <!--                            <button type="button" lay-submit="" lay-filter="serch" class="layui-btn diabtn"-->
        <!--                                    >查询-->
        <!--                            </button>-->
        <!--                        </div>-->
        <!--                    </div>-->
        <!--                </form>-->
        <!--            </div>-->
        <!--        </div>-->
        <!--        &lt;!&ndash;周边搜索弹窗end&ndash;&gt;-->
    </div>
</template>


<script>
    import * as mUtil from '@/utils/plugin.js';
    import * as mConfig from '@/utils/config.js';
    import * as api from '@/api/api.js' //页面中引入
    import $ from 'jquery'
    import axios from 'axios'
    import md5 from "js-md5";
    import sha512 from "js-sha512";
    import DialogZone from "./dialogZone";

    export default {

        name: 'emergencyDisposal',

        components: {DialogZone},

        data() {
            return {
                // zbWidth:'',//周边搜索宽度
                // zbHeight:'',//周边搜索高度
                bridgeData: {},//当前桥梁详情信息
                tunnelData: {},//当前隧道详情信息
                map: null,
                mConfig: mConfig.getMyConfig(), //一些配置信息
                eventArr: [], //所有事件信息，第一条为默认展示数据
                currentEventId: '', //当前信息展示的事件id
                currentEventRoad: '', //当前事件的路线信息code
                currentPoint: null, //当前事件的事件点
                paneIndex: 0,//判断选中的是哪一个选项卡
                searchAroundDialogVisible: false, //周边搜索弹窗显示
                aroundContentDialogVisible: false, //应急站点等周边搜索内容弹窗显示
                aroundTitle: '', //周边搜索内容弹窗标题
                aroundParam: {}, //周边搜索内容类型
                stormeroomInfo: {}, //站点详细信息
                stormeroomTable: {storeroomCode: ''}, //站点表格请求参数
                imgUrl: "../../static/img/icon/map",
                coverShow: false,//弹出层遮盖
                radius: 50,
                //缓冲区图标
                searchAroundTypeArr: [
                    {name: '站点', src: '/event/yjzd.png', type: '1', active: false},
                    {name: '固定视频', src: '/event/gdsp.png', type: '2', active: false},
                    {name: '情报板', src: '/event/qbb.png', type: '3', active: false},
                    {name: '车载', src: '/event/cz_online.png', type: '4', active: false},
                    {name: '无人机', src: '/event/frj_online.png', type: '5', active: false},
                    {name: '桥梁', src: '/event/ql.png', type: '6', active: false},
                    {name: '隧道', src: '/event/sd.png', type: '7', active: false},
                ],
                //地图标点图标
                aroundIcon: {
                    icon1: '../../static/img/icon/map/event/yjzd.png',//应急站点
                    icon2: '../../static/img/icon/map/event/gdsp.png',
                    icon3: '../../static/img/icon/map/event/qbb.png',//情报板
                    icon4: '../../static/img/icon/map/event/cz_online.png',
                    icon5: '../../static/img/icon/map/event/frj_online.png',
                    icon6: '../../static/img/icon/map/event/ql.png',//桥梁
                    icon7: '../../static/img/icon/map/event/sd.png',//隧道
                },
                aroundArr: [], //周边搜索数据

                layerNames: {
                    aroundLayer: 'aroundLayer', //周边搜索图层
                    eventLayer: 'eventLayer', //事件图层
                    currentEventLayer: 'currentEventLayer', //当前事件图层
                    circleLayer: 'CircleLayer' //缓冲区图层
                },

                clearAroundBtn: false, //清除缓冲区按钮是否显示

                //视频播放配置
                //视频播放配置
                playerOptions: {
                    live: true,
                    autoplay: true, // 如果true，浏览器准备好时开始播放
                    bigPlayButton: false,//播放按钮
                    muted: false, // 默认情况下将会消除任何音频
                    loop: false, // 是否视频一结束就重新开始
                    preload: 'auto', // 建议浏览器在<video>加载元素后是否应该开始下载视频数据。auto浏览器选择最佳行为,立即开始加载视频（如果浏览器支持）
                    // aspectRatio: '16:9', // 将播放器置于流畅模式，并在计算播放器的动态大小时使用该值。值应该代表一个比例 - 用冒号分隔的两个数字（例如"16:9"或"4:3"）
                    poster: '../../static/img/icon/map/event/no_video.png', //你的封面地址
                    fluid: false, // 当true时，Video.js player将拥有流体大小。换句话说，它将按比例缩放以适应其容器。
                    width: '100%',
                    height: '100%',
                    controlBar: {
                        timeDivider: false,
                        durationDisplay: false,
                        remainingTimeDisplay: false,
                        currentTimeDisplay: false, // 当前时间
                        volumeControl: false, // 声音控制键
                        playToggle: false, // 暂停和播放键
                        progressControl: false, // 进度条
                        fullscreenToggle: true // 全屏按钮
                    },
                    sources: [{
                        // type: "video/mp4",
                        type: 'application/x-mpegURL',
                        // src: "http://1252093142.vod2.myqcloud.com/4704461fvodcq1252093142/48c8a9475285890781000441992/playlist.m3u8", //你的m3u8地址（必填）
                        src: "",
                        withCredentials: false
                    }],
                    flash: {hls: {withCredentials: false}},
                    html5: {hls: {withCredentials: false}},
                    // notSupportedMessage: '此视频暂无法播放，请稍后再试' // 允许覆盖Video.js无法播放媒体源时显示的默认信息。
                    notSupportedMessage: ' ' // 允许覆盖Video.js无法播放媒体源时显示的默认信息。
                },

                videoSrc: '', //视频播放地址
                videoLoading: false, //视频加载
                videoToken: '', //视频token
                videoTokenTimer: null, //视频token调用
                videoTokenTime: 30, //视频token调用秒数

                infoBoardArr: [], //情报板展示
                isXxfb: true, //true:信息发布接口  false:智慧路接口

                eventLegend: [], //事件图例
                aroundLegend: [] //周边搜索图例

            }
        },

        computed: {
            mUtil() {
                return mUtil.getMyUtil();
            },
            // player() {
            //   return this.$refs.videoPlayer.player;
            // }
            player: {
                get() {
                    return this.$refs.videoPlayer.player;
                },
                set(val) {
                    this.$refs.videoPlayer.player = val;
                }
            },
            zbWidth() {
                if (this.aroundParam.type == '2' || this.aroundParam.type == '4' || this.aroundParam.type == '5') {
                    return '800px'
                } else if (this.aroundParam.type == '3') {
                    return '30%'
                } else if (this.aroundParam.type == '6') {
                    return '600px'
                } else if (this.aroundParam.type == '7') {
                    return '600px'
                } else {
                    return '70%'
                }
            },
            zbHeight() {
                if (this.aroundParam.type == '2' || this.aroundParam.type == '4' || this.aroundParam.type == '5') {
                    return '700px'
                } else if (this.aroundParam.type == '3') {
                    return '600px'
                } else if (this.aroundParam.type == '6') {
                    return '500px'
                } else if (this.aroundParam.type == '7') {
                    return '300px'
                } else {
                    return 'auto'
                }
            }
        },

        created() {
        },
        mounted() {
            this.map = initTheme2Map('map', {initZoom: 6});
            this.getData();
        },

        methods: {
            /**
             * 得到所有事件数据
             */
            getData() {
                this.loading = true;
                api.eventList().then((res) => {
                    // this.eventArr = this.mUtil.getAxiosData(res, this);
                    this.eventArr = res.resultData

                    let points = [], //静态事件
                        ssp = [], //当前事件
                        sspPoints = [], //当前事件
                        polys = [];
                    if (this.eventArr.length > 0) {
                        this.currentEventId = this.eventArr[0]['fdObjectid'];
                        // this.currentEventRoad = this.eventArr[0]['roadCode'];
                        this.currentPoint = this.eventArr[0];
                    }

                    const eventLegend = {};

                    this.eventArr.forEach((d, index) => {
                        eventLegend['event_' + d.levelId + d.fromFlag] = {
                            sort: d.levelId + d.fromFlag,
                            name: d.levelIdValue + (d.fromFlag == '1' ? '响应' : '预警'),
                            color: d.color
                        };

                        if (d.eventPosition && d.eventPosition.indexOf('MULTIPOINT') > -1) {
                            const pos = d.eventPosition.substring(12, d.eventPosition.length - 2);
                            const isStatic = index == 0 ? '' : '_ly';
                            const img = this.imgUrl + "/label/" + d.color + isStatic + ".png";
                            const pointInfo = {
                                point: pos.split(" "), //--点坐标
                                attributes: {
                                    id: d.fdObjectid,
                                    name: d.eventName,
                                    lat: pos.split(" ")[0],
                                    lng: pos.split(" ")[1],
                                    tooltip: d.eventName,
                                    params: d,
                                    infoWindowWidth: "15vw",
                                    // infoWindowContent: ,
                                    infoWindowCallBack: this.getMapWinCallBack
                                },
                                icons: [{src: img}]
                            };
                            if (index == 0) {
                                // sspPoints.push(pointInfo);
                                points.push(pointInfo);
                                ssp.push({
                                    name: d.eventName,
                                    lnglat: pos.split(" "),//--点坐标
                                    color: d.color,
                                    type: 'circle',
                                    speed: 0.4,
                                    _map: this.map.getOLMap()
                                })

                            } else {
                                points.push(pointInfo);
                            }

                        } else if (d.eventPosition) {

                            let cs = 'rgba(22,125, 243, 0.5)';
                            if (d.color == 'red') cs = 'rgba(243, 14, 8, 0.5)';
                            if (d.color == 'orange') cs = 'rgba(249, 150, 10, 0.5)';
                            if (d.color == 'yellow') cs = 'rgba(250, 253, 2, 0.5)';

                            const polyInfo = {
                                wkt: d.eventPosition,
                                attributes: {
                                    id: d.fdObjectid,
                                    name: d.eventName,
                                    params: d,
                                    tooltip: d.eventName,
                                    infoWindowCallBack: this.getMapWinCallBack
                                },
                                // style: ['polygon_green','polygon_blue']
                                // style: [d.color, d.color]
                                style: [cs, d.color]
                            };
                            //先注释掉多边形的地图背景
                            // polys.push(polyInfo);
                        }
                    });

                    for (const key in eventLegend) {
                        this.eventLegend.push(eventLegend[key]);
                    }

                    this.eventLegend = this.mUtil.sortByKeyAsc(this.eventLegend, 'sort');

                    clearFlash(this.map);
                    drawFlash(this.map, ssp, 1);

                    addPoints(this.map, this.layerNames.eventLayer, true, points);
                    this.map.setLayerZIndex(this.layerNames.eventLayer, 3);

                    showWKTByColor(this.map, this.layerNames.eventLayer + '_polys', true, polys);
                    map.setLayerZIndex(this.layerNames.eventLayer + '_polys', 2);

                }).catch(err => {
                    this.mUtil.errorAxios(this.mConfig.toast("dataError"), this);
                });
            },
            /**wuyuxin新增table选中*/
            tabClick(tab, event) {
                this.paneIndex = tab.index;
            },
            /**
             * 点击事件回调方法
             * @param map
             * @param coordinate
             * @param feature
             */
            getMapWinCallBack(map, coordinate, feature) {
                const attributes = feature.get("attributes"); //属性对象
                const params = attributes.params;

                clearFlash(this.map);
                if (this.currentPoint.eventPosition && this.currentPoint.eventPosition.indexOf('MULTIPOINT') > -1) {
                    const imgStatic = this.imgUrl + "/label/" + this.currentPoint.color + "_ly.png";
                    updatePoints(this.map, this.layerNames.eventLayer, [{
                        id: this.currentPoint.fdObjectid,
                        icons: [{src: imgStatic}]
                    }]);
                }
                if (params.eventPosition && params.eventPosition.indexOf('MULTIPOINT') > -1) {
                    const img = this.imgUrl + "/label/" + params.color + ".png";

                    const pos = params.eventPosition.substring(12, params.eventPosition.length - 2);
                    let ssp = [];
                    ssp.push({
                        name: params.eventName,
                        lnglat: pos.split(" "),//--点坐标
                        color: params.color,
                        type: 'circle',
                        speed: 0.4,
                        _map: this.map.getOLMap()
                    });
                    drawFlash(this.map, ssp, 1);

                    let pointsUpdate = [{
                        id: params.fdObjectid,
                        icons: [{src: img}]
                    }];

                    updatePoints(this.map, this.layerNames.eventLayer, pointsUpdate);
                }


                this.currentEventId = params.fdObjectid;
                // this.currentEventRoad = params.roadCode;
                this.currentPoint = params;
                this.coverShow = true;
                this.searchAroundDialogVisible = true;
            },

            /**
             * 关闭周边搜索弹窗
             */
            cancelSearchAroundDialog() {
                this.coverShow = false;
                this.searchAroundDialogVisible = false;
                this.clearAround();
            },

            /**
             * 关闭周边搜索内容弹窗
             */
            closeAroundContentDialog() {
                this.aroundContentDialogVisible = false;
                clearInterval(this.videoTokenTimer);
            },

            inputNum() {
                // value=value.replace(/^(([1-9][0-9]*)|(([0]\.\d{1,2}|[1-9][0-9]*\.\d{1,2})))/g,'')
                // this.radius = this.radius.replace(/^(([1-9][0-9]*)|(([0]\.\d{1,2}|[1-9][0-9]*\.\d{1,2})))/g,'');
                this.radius = this.radius.replace(/[^\d.]/g, "");  //清除“数字”和“.”以外的字符

                this.radius = this.radius.replace(/^\./g, "");  //验证第一个字符是数字而不是.

                this.radius = this.radius.replace(/\.{2,}/g, "."); //只保留第一个. 清除多余的.

                this.radius = this.radius.replace(".", "$#$").replace(/\./g, "").replace("$#$", ".");
            },

            /**
             * 查询周边搜索
             */
            submitSearchAroundDialog(msg) {
                this.radius = msg || 50;
                const radius = msg * 1000;
                let type = [];
                this.searchAroundTypeArr.forEach((d) => {
                    if (d.active) {
                        type.push(d.type);

                        if ('2,4,5'.indexOf(d.type) > -1) {
                            this.loginVideo();
                        }
                    }

                });

                if (type.length == 0) {
                    this.mUtil.errorAxios('请选择周边类型', this);
                    return false;
                }

                const params = {
                    longitude: this.currentPoint.longitude,
                    latitude: this.currentPoint.latitude,
                    radius: this.radius,
                    type: type.join(',')
                };

                this.loading = true;
                api.getRange(params).then((res) => {
                    this.coverShow = false;
                    this.searchAroundDialogVisible = false;
                    createCircle(this.map, this.currentPoint.longitude, this.currentPoint.latitude, radius, 'rgba(0, 218, 180, .5)', 'rgba(15, 216, 181, 1)');
                    if (res.code == 200) {
                        this.aroundArr = res.resultData
                    } else {
                        layer.msg(res.msg)
                    }

                    this.clearAroundBtn = true;
                    let points = [];
                    this.aroundArr.forEach((d) => {

                        const pointInfo = {
                            point: [+d.longitude, +d.latitude], //--点坐标
                            attributes: {
                                id: d.fdObjectid,
                                params: d,
                                infoWindowWidth: "15vw",
                                // infoWindowContent: ,
                                infoWindowCallBack: this.getAroundCallBack
                            },
                            icons: [{src: this.aroundIcon['icon' + d.type]}]
                        };

                        points.push(pointInfo);
                    });

                    addPoints(this.map, this.layerNames.aroundLayer, true, points);

                    this.map.setLayerZIndex(this.layerNames.aroundLayer, 2);

                }).catch(err => {
                    this.coverShow = false;
                    this.searchAroundDialogVisible = false;
                    this.mUtil.errorAxios(this.mConfig.toast("dataError"), this);
                });

            },
            submitSearchAroundDialogS(data) {
                if (data[0] != 'error') {
                    let res = data[0]
                    let radius = data[1]
                    this.coverShow = false;
                    this.searchAroundDialogVisible = false;
                    createCircle(this.map, this.currentPoint.longitude, this.currentPoint.latitude, radius, 'rgba(0, 218, 180, .5)', 'rgba(15, 216, 181, 1)');
                    this.aroundArr = res.resultData
                    this.clearAroundBtn = true;
                    let points = [];
                    this.aroundArr.forEach((d) => {
                        const pointInfo = {
                            point: [+d.longitude, +d.latitude], //--点坐标
                            attributes: {
                                id: d.fdObjectid,
                                params: d,
                                infoWindowWidth: "15vw",
                                // infoWindowContent: ,
                                infoWindowCallBack: this.getAroundCallBack
                            },
                            icons: [{src: this.aroundIcon['icon' + d.type]}]
                        };

                        points.push(pointInfo);
                    });

                    addPoints(this.map, this.layerNames.aroundLayer, true, points);

                    this.map.setLayerZIndex(this.layerNames.aroundLayer, 2);
                } else {
                    this.coverShow = false;
                    this.searchAroundDialogVisible = false;
                    this.mUtil.errorAxios(this.mConfig.toast("dataError"), this);
                }

            },
            /**
             * 周边搜索图标点击回调
             * @param map
             * @param coordinate
             * @param feature
             */
            getAroundCallBack(map, coordinate, feature) {
                const attributes = feature.get("attributes"); //属性对象
                const params = attributes.params;
                this.aroundParam = params;
                const fun = eval('this.showAround' + params.type);
                fun.call(this, params);
            },
            /**
             * 打开周边搜索弹窗
             * @param title 弹窗标题
             */
            showAroundDialogWin(title) {
                this.aroundTitle = title;
                this.aroundContentDialogVisible = true;
            },

            /**
             * 周边搜索站点点击
             */
            showAround1(params) {
                this.showAroundDialogWin('应急站点');
                this.stormeroomTable.storeroomCode = params.fdObjectid; //站点表格请求参数

                const param = new FormData();
                param.append('fdObjectid', params.fdObjectid);
                axios.all([
                    api.materialStoreroom(params), //站点信息
                ]).then(axios.spread((stormeroomRes) => {
                    this.stormeroomInfo = this.mUtil.getAxiosData(stormeroomRes, this)[0];

                })).catch(err => {
                    this.mUtil.errorAxios(this.mConfig.toast("dataError"), this);
                });
            },


            /**
             * 周边搜索固定视频点击
             */
            showAround2(params) {
                /*
                        this.showAroundDialogWin('固定视频');
                        this.videoSrc = 'rtmp://58.200.131.2:1935/livetv/hunantv';
                        this.playerOptions.sources[0].src = 'rtmp://58.200.131.2:1935/livetv/hunantv';
                */
                if (this.videoToken) {
                    this.getVideoPath('固定视频');
                } else {
                    this.mUtil.errorAxios('视频登录失败', this);
                }


            },

            /**
             * 视频登录
             */
            loginVideo(params) {
                var param = {user: this.$video_user, pwd: this.$video_pwd};
                var _this = this;

                const url = this.$rtmp + '/vmslogin?user=' + this.$video_user + '&pwd=' + this.$video_pwd;
                $.ajax({
                    url: url,
                    type: 'POST',
                    data: param,
                    // async:false,
                    dataType: 'json',
                    contentType: false,
                    processData: false,
                    success: function (data) {

                        if (data.result == 0) {
                            _this.videoToken = data.token;
                            _this.videoTokenTimer = setInterval(() => {
                                _this.refreshVideoToken();
                            }, 1000 * _this.videoTokenTime);

                        } else {
                            _this.mUtil.errorAxios('视频登录失败', _this);
                        }
                    },
                    error: function (error) {
                        _this.mUtil.errorAxios('视频登录失败', _this);
                    }
                });

                // this.loading = true;
                // this.$axios.post('http://36.2.8.1:81/vmslogin', {user: 'webvideo', pwd: 'sinovms825'}).then((res) => {
                // this.$axios.post('http://36.2.8.1:81/vmslogin', params).then((res) => {
                // this.$axios.post('/api/vmslogin?user=webvideo&pwd=sinovms825', JSON.stringify({"user": "webvideo", "pwd": "sinovms825"}), {
                /*this.$axios.post('http://36.2.8.1:81/vmslogin?user=webvideo&pwd=sinovms825', JSON.stringify({"user": "webvideo", "pwd": "sinovms825"}), {
                  headers: {
                    'X-Token': '',
                  }
                }).then((res) => {
                // this.$axios.post(this.$video + '/api/vmslogin', param).then((res) => {
                // this.$axios.post('/api/vmslogin', param).then((res) => {

                  console.log(JSON.stringify(res))
                }).catch(err => {
                  this.mUtil.errorAxios(this.mConfig.toast("dataError"), this);
                });*/

                /*
         */
            },

            /**
             * 刷新视频token
             */
            refreshVideoToken() {
                const url = this.$rtmp + '/vmskeepalive?token=' + this.videoToken;
                const param = {token: this.videoToken};
                const _this = this;
                $.ajax({
                    url: url,
                    type: 'POST',
                    data: param,
                    // async:false,
                    dataType: 'json',
                    contentType: false,
                    processData: false,
                    success: function (data) {
                        if (data.result == 0) {


                        } else {

                        }
                    },
                    error: function (error) {

                    }
                });

                // this.$axios.post('http://36.2.8.1:81/vmskeepalive', {token: this.videoToken}).then((res) => {
                //
                //   console.log(JSON.stringify(res))
                // }).catch(err => {
                //   this.mUtil.errorAxios(this.mConfig.toast("dataError"), this);
                // });
            },

            /**
             * 得到视频播放路径
             */
            /**
             * 得到视频播放路径
             */
            getVideoPath(title) {
                // const videoId = '36010000041310000002';
                const videoId = this.aroundParam.platformNo;
                // const url = 'http://36.2.8.1:81/vmslive?vid=' + this.aroundParam.platformNo + '&cmd=play&token=' + this.videoToken;
                const url = this.$rtmp + '/vmslive?vid=' + videoId + '&cmd=play&type=hls&token=' + this.videoToken;
                // const param = {vid: this.aroundParam.platformNo, cmd: 'play', token: this.videoToken};
                const _this = this;
                $.ajax({
                    url: url,
                    type: 'GET',
                    // data: param,
                    async: false,
                    dataType: 'json',
                    contentType: false,
                    processData: false,
                    success: function (data) {
                        // data = {"msg":"成功","playurl":"http://36.2.8.1:8026/hls/36010000041310000005-1.m3u8?secret=5b5102f68586dd1698410d45a0544cda3b0db06f202a770f6916f39ad2625466c1ca4b34603c4f4e8099c6a98f8c9e291a3c57285800088945b772a3e17d1705","result":0};
                        if (data.result == 0) {
                            // data.playurl = 'http://36.2.8.1:8026/hls/36010000041310000005-1.m3u8?secret=e76407a678c5ef29b44186a7d823f8e2d994ad385d62bd05dd80562253d05e840bf944f64ce82e52de42354199a8e4baf1f46c3db9d3e859f75d7e5717a21605'
                            // data.playurl = 'http://36.2.8.1:8026/hls/36010000041310000005-1.m3u8?secret=bc4dcd6338fea20fec15448f018ab5f5e69bc611581b1a8aa95f7322a8c1660a29fc1c33ac179e512e4800c88ba822c5e489ff0405bcc705310b1c2908022ff9'
                            _this.videoSrc = data.playurl;
                            // _this.videoLoading = true;
                            _this.playerOptions.sources[0].src = data.playurl;
                            _this.showAroundDialogWin(title);
                        } else {
                            _this.getMainPlayUrl(title);
                        }
                    },
                    error: function (error) {

                    }
                });

                // _this.mUtil.errorAxios('视频登录失败', _this);
                // this.$axios.post('http://36.2.8.1:26020/vmslive', {vid: this.aroundParam.platformNo, cmd: 'play', token: this.videoToken}).then((res) => {
                //   console.log(JSON.stringify(res))
                // }).catch(err => {
                //   this.mUtil.errorAxios(this.mConfig.toast("dataError"), this);
                // });
            },

            /**
             * 子码流失败后重新获取主码流
             */
            getMainPlayUrl(title) {

                let self = this;
                $.get(
                    `${this.$rtmp}/vmslive?vid=${this.aroundParam.platformNo}&cmd=play&type=hls&idx=1&token=` + this.videoToken, (data) => {
                        // console.log(JSON.stringify(data))
                        if (data.result === 0) {

                            self.videoSrc = data.playurl;
                            self.playerOptions.sources[0].src = data.playurl;
                            self.showAroundDialogWin(title);

                            // setInterval(() => {
                            //   self.playerOptions.sources[0].src = data.playurl;
                            // }, 1000 * 3);

                        } else {

                            self.$message({
                                type: `error`,
                                message: data.msg,
                            });
                        }
                    }
                );
            },


            /**
             * 周边搜索情报板点击
             */
            showAround3(params) {
                if (params.isSystemRelease == '0') { //智慧路
                    this.getWisdomData();
                } else { //1为信息发布
                    this.getXxfbData();
                }
            },

            /**
             * 智慧路的信息发布数据
             */
            getWisdomData() {
                /*const res = {
                  "result": true,
                  "msg": "请求成功",
                  "device_id": " e064a7f5a11147ab862e21a0ec01fb8d",
                  "list": [
                    {
                      "img": "data:image/jpg;base64,",
                      "flag": "0",
                      "text": "下雨路滑，减速慢行"
                    }
                  ]
                }

                if(res.result) {
                  this.infoBoardArr = res.list;
                } else {
                  this.mUtil.errorAxios(res.msg, this);
                }*/
                // this.aroundParam.platformNo = 'e064a7f5a11147ab862e21a0ec01fb8d';
                // const url = this.$board + '/jx_publish/queryDevicePublish';
                // const url = '/api/jx_publish/queryDevicePublish';
                // const param = {device_id: this.aroundParam.platformNo};
                const param = {platformNo: this.aroundParam.platformNo};
                this.infoBoardArr = [];
                // this.$axios.post('http://36.2.24.111:8899/jx_publish/queryDevicePublish', {device_id: this.aroundParam.platformNo}).then((res) => {
                // this.$axios.post('/proxy_info/jx_publish/queryDevicePublish', param).then((res) => {
                api.selectBookConcent(param).then((res) => {
                    this.infoBoardArr = this.mUtil.getAxiosData(res, this);
                    if (this.infoBoardArr.length > 0) this.showAroundDialogWin('情报板');
                    else this.mUtil.errorAxios('情报板无数据', this);

                    /*if(res.result) {
                      this.infoBoardArr = res.list;
                      if(res.list.length>0) this.showAroundDialogWin('情报板');
                      else this.mUtil.errorAxios('情报板无数据', this);
                    } else {
                      this.mUtil.errorAxios(res.msg, this);
                      this.aroundContentDialogVisible = false;
                    }*/
                }).catch(err => {
                    this.mUtil.errorAxios(this.mConfig.toast("dataError"), this);
                    this.aroundContentDialogVisible = false;
                });
            },

            /**
             * 信息发布的数据
             */
            getXxfbData() {
                this.infoBoardArr = [];
                // var params = {infoboardId: this.aroundParam.platformNo}
                var params = new FormData();
                params.append('infoboardId', this.aroundParam.platformNo)
                this.$axios.post(this.$xxfb + '/infoboardCurrentInfo/get', params).then((res) => {
                    res = res.data;
                    res.forEach((d) => {

                        let flag = '1';
                        if (d.hasOwnProperty('fileUrl')) flag = '0';
                        d.flag = flag;
                        d.width = d.specification.split("*")[0] + 'Px';
                        d.height = d.specification.split("*")[1] + 'Px';

                        this.infoBoardArr.push(d);
                    });

                    if (res.length > 0) this.showAroundDialogWin('情报板');
                    else this.mUtil.errorAxios('情报板无数据', this);

                }).catch(err => {
                    this.mUtil.errorAxios(this.mConfig.toast("dataError"), this);
                });

            },


            /**
             * 周边搜索车载点击
             */
            showAround4(params) {
                if (this.videoToken) {
                    this.getVideoPath('车载');
                } else {
                    this.mUtil.errorAxios('视频登录失败', this);
                }
                // this.showAroundDialogWin('车载');
            },


            /**
             * 周边搜索无人机点击
             */
            showAround5(params) {
                // this.showAroundDialogWin('无人机');
                if (this.videoToken) {
                    this.getVideoPath('无人机');
                } else {
                    this.mUtil.errorAxios('视频登录失败', this);
                }
            },
            /**
             * 周边搜索桥梁点击
             */
            showAround6(params) {
                this.showAroundDialogWin('桥梁');
                let par = {
                    uid: params.fdObjectid
                }
                api.selectQl(par).then(res => {
                    if (res.data.code === 200) {
                        let data = res.data.resultData
                        this.bridgeData = data //桥梁详情信息
                        // this.aroundContentDialogVisible = true
                        this.aroundTitle = data.qlmc + '详情'
                    } else {
                        this.mUtil.errorAxios('数据请求失败', this);
                    }
                })
            },

            /**
             * 周边搜索隧道点击
             */
            showAround7(params) {
                this.showAroundDialogWin('隧道');
                let par = {
                    uid: params.fdObjectid
                }
                api.selectSd(par).then(res => {
                    if (res.data.code === 200) {
                        let data = res.data.resultData
                        this.tunnelData = data //隧道详情信息
                        this.aroundTitle = data.sdmc + '详情'
                    } else {
                        thismUtil.errorAxios('数据请求失败', this);
                    }
                })
            },
            /**
             * 改变周边搜索选中状态
             * @param index
             */
            chengeAroundChecked(index) {
                this.searchAroundTypeArr[index].active = !this.searchAroundTypeArr[index].active;
            },

            /**
             * 清除缓冲区
             */
            clearAround() {
                clearLayerByMap(this.map, this.layerNames.aroundLayer);
                clearLayerByMap(this.map, this.layerNames.circleLayer);
                this.clearAroundBtn = false;
                this.searchAroundTypeArr.map((d) => {
                    d.active = false;
                    return d;
                });

                this.radius = 50;
            },

            /**
             * 播放开始
             * @param player
             */
            onPlayerWaiting(player) {
                // console.log("播放....");
                this.videoLoading = false;
            },

            /**
             * 加载完成
             * @param player
             */
            onPlayerLoadeddata(player) {
                // console.log("加载完成....")
                this.videoLoading = false;
            },

            /**
             * 加载中
             * @param player
             */
            onPlayerCanplaythrough(player) {
                // console.log("加载中....")
                // this.videoLoading = true;
            },

        },

        watch: {},

        filters: {}

    }

</script>
<style lang="less">
    body .overs .el-scrollbar__wrap {
        overflow-x: hidden;
    }

    .emergency_disposal {
        #map {
            .ol-tooltip.ol-tooltip-static {
                color: rgb(255, 255, 255);
                padding: 5px 10px;
                margin-left: -10px;
                background: rgba(0, 0, 0, 0.3);
                border-radius: 5px;
            }
        }
    }
</style>

<style lang="less" scoped>
    @import "../../static/css/my_style";

    @bgImg: 'data:image/jpg;base64,';
    @box: 'data:image/png;base64,';
    @dialog: 'data:image/png;base64,';
    .cover_layer {
        position: absolute;
        top: 0px;
        left: 0px;
        width: 100%;
        height: 100%;
        background: rgba(0, 0, 0, 0.5);
        z-index: 9;
    }

    .map_dialog {
        width: 402px;
        height: auto;
        z-index: 99;
        position: absolute;
        left: 50%;
        top: 50%;
        transform: translate(-50%, -50%);
        background-image: url("@{dialog}");
        background-repeat: no-repeat;
        background-size: 100% 100%;
        background-position: center;
        padding: 10px;
        border: none;
        box-shadow: none;

        &.zkr-dialog {
            /deep/ .el-dialog {


                /deep/ .zkr-dialog-btn.el-button {
                    background: #4379b1;
                    border: none;
                    color: #ffffff;
                }
            }

            /deep/ .el-dialog__header, /deep/ .el-dialog {
                background-color: transparent;

                .el-dialog__title {
                    color: #50ace7;
                }

            }

            .search_around_box {
                .header_box {
                    display: flex;
                    justify-content: space-between;
                    align-items: center;

                    .spantitle {
                        height: 50px;
                        line-height: 50px;
                        color: #50ace7;
                        padding-left: 10px;
                        font-size: 17px;
                    }

                    .header_close {
                        padding-right: 10px;
                        cursor: pointer;
                        color: #909399;
                    }

                    .header_close:hover {
                        color: #50ace7;
                    }
                }

                /deep/ .slider {
                    margin: 10px 0px;

                    .demo-slider {
                        width: auto;
                        padding: 0px 15px;

                        .layui-slider {
                            background: #2d88b3;
                        }
                    }
                }

                .input_flex {
                    display: flex;
                    justify-content: space-evenly;
                    padding: 0px 10px;
                    width: auto;
                    margin: 10px 0px;
                    /* 谷歌 */

                    input::-webkit-outer-spin-button,
                    input::-webkit-inner-spin-button {
                        -webkit-appearance: none;
                        appearance: none;
                        margin: 0;
                    }

                    /* 火狐 */

                    input {
                        -moz-appearance: textfield;
                    }

                    input {
                        background: rgba(0, 0, 0, 0.3);
                        border: 1px solid #0468a6;
                        color: #fff;
                    }

                    /deep/ .layui-input:hover, .layui-textarea:hover {
                        border: 1px solid #0468a6 !important;
                        color: #fff;
                    }

                    /deep/ .layui-input:focus, .layui-textarea:focus {
                        border: 1px solid #0468a6 !important;
                        color: #fff;
                    }

                    .show_title {
                        width: 100px;
                        height: 38px;
                        border-top-right-radius: 3px;
                        border-bottom-right-radius: 3px;
                        background: #4379b1;
                        color: #fff;
                        text-align: center;
                        line-height: 38px;
                    }


                }

                .diabtn {
                    background: #4379b1;
                }

                .content {
                    display: flex;
                    justify-content: start;
                    flex-wrap: wrap;

                    .around_item_box {
                        //height: 82Px;
                        height: 30px;
                        margin: 4px 8px;
                        margin-bottom: 15px;

                        .around_count_box {
                            width: 106px;
                            height: 25px;
                            display: flex;
                            border-radius: 5px;
                            border: 1px dashed #0468a6;
                            padding: 5px;
                            cursor: pointer;
                            color: #50ace7;
                            position: relative;

                            &.active {


                                .around_name {
                                    color: #ffffff;
                                }
                            }

                            &:hover {
                                .around_name {
                                    color: #ffffff;
                                }
                            }

                            .around_img, .around_name {
                                width: 100%;
                                text-align: end;

                                font-size: 12Px;
                            }

                            .around_name {
                                text-align: start;
                                line-height: 27Px;
                                margin-left: 10Px;
                            }

                            .around_img {
                                img {
                                    //width: 54Px;
                                    //height: 54Px;
                                }
                            }
                        }
                    }
                }

            }

            /deep/ .around_input_box {
                width: 100%;
                padding: 5Px 5Px 10Px 5Px;

                .el-input__inner {
                    background-color: rgba(0, 0, 0, .3);
                    border: 1px solid #0468a6;
                    color: #ffffff;
                }

                .el-input-group__append {
                    background-color: #4379b1;
                    color: #ffffff;
                    border: 1px solid #0468a6;
                }
            }

            /*/deep/ .zkr-dialog .el-dialog__body{*/
            /*  width: calc(100% + 28px);*/
            /*}*/

            /deep/ .tabs .el-tabs__header .el-tabs__item {
                color: #fff !important;
            }

            /*站点样式*/

            .station_box {
                height: 60vh;
                /**/

                /deep/ .el-tabs--card > .el-tabs__header {
                    border-bottom: 1px dashed #007ec4;

                    .el-tabs__nav {
                        border-color: #007ec4;
                        border-radius: 0;
                    }


                    .el-tabs__item {
                        background: #021435;
                        color: #ffffff;
                        border-color: #007ec4;

                        &.is-active {
                            background: #3687d0;
                            border-color: #007ec4;
                        }
                    }
                }

                .form_info {
                    color: #ffffff;
                    line-height: 50Px;
                    .text_nowrap;

                    &.info_filed {
                        text-align: right;
                    }
                }

            }

            /*视频样式*/

            .video_box {
                height: 100%;

                /deep/ .vjs-error .vjs-error-display:before {
                    content: '';
                }

                /deep/ .video-player {

                    > div {
                        width: 100%;
                        height: 100% !important;

                        &:first-child {
                            height: 100% !important;
                        }
                    }

                    .vjs-big-play-button {
                        display: none;
                    }
                }
            }

            /*情报板样式*/

            .info_board_box {
                height: 50vh;

                .info_board_content {
                    width: 100%;
                    height: 100%;
                    background: #000000;

                    /*信息发布*/

                    .info__text {
                        position: relative;
                        transform: scale(2) translateY(-50%);
                        top: 50%;
                        display: block;
                        margin: auto;

                        .info_item {
                            position: absolute;
                        }
                    }

                    /*智慧路*/

                    .info_text {
                        color: #ffffff;
                        height: 100%;
                        text-align: center;
                        font-size: .5rem;

                        &:before {
                            display: inline-block;
                            content: "";
                            height: 100%;
                            vertical-align: middle;
                        }
                    }

                    img {
                        height: 100%;
                        display: block;
                        margin: auto;
                    }
                }

            }
        }

    }

    .zbss_dialog {
        /deep/ .el-dialog__body {
            height: 100% !important;
            padding: 0px !important;
            width: 100% !important;
        }

        /deep/ .el-dialog {
            width: 100%;
        }

        /deep/ .el-dialog__body::-webkit-scrollbar {
            width: 0px !important;
        }

        /deep/ .el-dialog__footer {
            padding: 0px 20Px 7Px 20Px !important;
        }
    }

    .emergency_disposal {
        background: url('@{bgImg}') no-repeat center center;
        background-size: cover;

        .legend_box {
            position: absolute;
            bottom: 25Px;
            left: 21%;
            color: #ffffff;
            background: rgba(0, 0, 0, .3);

            .legend_header {
                text-align: center;
                line-height: 30Px;
                background: #10214f;
                font-size: 16px;
                padding: 5px 0;
            }

            .legend_body {
                padding: 5Px 15Px;

                .legend_item {
                    display: flex;

                    .clear_float {
                        clear: both;
                    }

                    .legend_color, .legend_icon, .legend_name {
                        display: inline-block;
                        float: left;
                        margin: 10px;
                        font-size: 12px;
                    }

                    .legend_color {
                        width: 40px;
                        height: 15px;
                    }

                    .legend_icon {
                        width: 24Px;
                        height: 24Px;

                        img {
                            height: 100%;
                            display: block;
                            margin: auto;
                        }
                    }
                }
            }


        }

        .clear_around_btn {
            position: fixed;
            top: 5px;
            right: 135Px;
            z-index: 3;
            padding: 0;
            height: 20px;

            /deep/ .el-button {
                font-size: 17Px;
                color: #ffffff;
                padding: 0;
            }
        }

        #map {
            width: calc(100% - 220px);
            margin: 0 10px 10px 10px;
            height: calc(100% - 70px); //- 0.26042rem
            position: absolute;
            z-index: 0;
            top: 60px;


            background-size: 100% 100%;
            background-repeat: no-repeat;

            .ol-tooltip.ol-tooltip-static {
                color: rgb(255, 255, 255);
                padding: 5px 10px;
                margin-left: -10px;
                background: rgba(0, 0, 0, 0.3);
                border-radius: 5px;
            }
        }

        .center_box {
            position: absolute;
            top: 50px;
            width: 40%;
            max-width: 700px;
            margin: auto;
            z-index: 1;
            left: 0;
            right: 0;

            .num_box {
                width: 33.3333333%;
                display: inline-block;
                float: left;
            }

            .center_title {

            }
        }

        .left_box, .right_box {
            width: 20%;
            height: calc(100% - 50px);
            overflow: hidden;
            position: absolute;
            z-index: 1;
            top: 30px;
            flex: 1;
            display: flex;
            flex-flow: column;
        }

        .left_box {
            left: 0;

            .duty {
                max-height: 15vh;
                margin-bottom: 15Px;
            }
        }

        .right_box {
            right: 0;

            .disposal_max_h {
                height: calc(100% - 270Px);

            }
        }


    }
</style>
